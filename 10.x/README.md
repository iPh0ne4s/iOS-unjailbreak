...
### 10.0-10.3.3_normal.sh
- `10.0-10.3.3_normal.sh` is supposed to be executed in normal mode
...
- Copy `10.0-10.3.3_normal.sh` to device, in my case it is copied to /private/var/tmp, set 777 permission
...
### 10.0-10.3.3_ramdisk.sh
- `10.0-10.3.3_ramdisk.sh` is supposed to be executed in SSH ramdisk mode
...
- On 10.0-10.2.1 run `/sbin/mount_hfs /dev/disk0s1s1 /mnt1` to mount system partition, on 10.3.x run `/sbin/mount_apfs /dev/disk0s1s1 /mnt1`
- Use FileZilla to access device, default is sftp://127.0.0.1, root, alpine, 6414. Change 6414 to 2222 if using SSHRD_Script
- Copy `10.0-10.3.3_ramdisk.sh` to device, in my case it is copied to /mnt1, set 777 permission
