...
cd /private/etc && rm -rf alternatives apt default dpkg pam.d profile.d rc.d ssh ssl profile symlibs.dylib
...
cd /usr/libexec && rm -rf cydia filza gnupg MSUnrestrictProcess _rocketd_reenable afc2d awk bigram code frcode lockdownd rmt rocketd sftp-server ssh-keysign ssh-pkcs11-helper sshd-keygen-wrapper substrate vndevice && mv lockdownd_o lockdownd && chmod 0755 lockdownd
