...
# Require manual replacement: /private/etc/fstab /usr/libexec/installd /usr/libexec/lsd
...
cd / && rm -rf .fseventsd DeveloperPatch User boot lib mnt taig
cd /Applications && rm -rf Cydia.app Filza.app MTerminal.app SpringtomizeApp.app cydown.app
cd /Library && mv LaunchDaemons/* /System/Library/LaunchDaemons && rm -rf Frameworks/CydiaSubstrate.framework LaunchDaemons MobileSubstrate NanoLaunchDaemons PreferenceBundles PreferenceLoader Switches
cd /System/Library && rm -rf Caches/com.apple.dyld/enable-dylibs-to-override-cache KeyboardDictionaries LaunchDaemons/com.openssh.sshd.plist LaunchDaemons/com.rpetrich.rocketbootstrapd.plist LaunchDaemons/com.saurik.Cydia.Startup.plist LaunchDaemons/com.tigisoftware.filza.helper.plist PreferenceBundles/AppList.bundle PrivateFrameworks/MobileSoftwareUpdate.framework/Support/softwareupdated && mv LaunchDaemons/com.apple.softwareupdateservicesd.plist__ LaunchDaemons/com.apple.softwareupdateservicesd.plist && mv PrivateFrameworks/MobileSoftwareUpdate.framework/Support/softwareupdated_0 PrivateFrameworks/MobileSoftwareUpdate.framework/Support/softwareupdated
cd /private/etc && rm -rf alternatives apt default dpkg pam.d profile profile.d rc.d ssh ssl symlibs.dylib
...
cd /bin && rm -rf bash bunzip2 bzcat bzip2 bzip2recover cat chgrp chmod chown cp date dd dir echo egrep false fgrep grep gunzip gzexe gzip kill ln ls mkdir mknod mktemp mv pwd readlink rm rmdir run-parts sed sh sleep stty su sync tar touch true uname uncompress vdir zcat zcmp zdiff zegrep zfgrep zforce zgrep zless zmore znew